...
export default async function SessionsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const query = parseAdminListQuery(params, ["startsAt", "createdAt", "status"]);
  const statusValue = typeof params.status === "string" ? params.status : Array.isArray(params.status) ? params.status[0] : undefined;
  const status = Object.values(AppointmentStatus).includes(statusValue as AppointmentStatus) ? statusValue as AppointmentStatus : undefined;
  const data = await listAdminAppointments(query, { status }, session);
  const currentParams = Object.fromEntries(Object.entries(params).map(([key, value]) => [key, Array.isArray(value) ? value[0] : value]));

  return <div className="admin-page-stack">
    <AdminPageHeader eyebrow="Appointment" title="جلسات" description="فهرست جلسات و وضعیت فعلی آن‌ها از داده‌های واقعی سیستم." />
    <section className="admin-panel-card">
      <AdminListToolbar>
        <AdminSearchInput defaultValue={query.search} placeholder="ایمیل، نام، دوره یا متخصص" />
        <label className="admin-search-field"><span>وضعیت</span><AdminSelect name="status" defaultValue={status ?? ""} ariaLabel="وضعیت جلسه" options={[{ value: "", label: "همه" }, ...Object.values(AppointmentStatus).map((value) => ({ value, label: statusLabel[value] }))]} /></label>
      ...
      {data.rows.length ? <AdminDataTable rows={data.rows} getRowKey={(row) => row.id} columns={[
        ...
        { key: "startsAt", label: "زمان", render: (row) => <time dateTime={row.startsAt.toISOString()}>{row.startsAt.toLocaleString("fa-IR-u-ca-gregory", { year: "numeric", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}</time> },
        { key: "status", label: "وضعیت", render: (row) => <AdminStatusBadge tone={statusTone(row.status)}>{statusLabel[row.status]}</AdminStatusBadge> },
        ...(canManage ? [{ key: "actions", label: "عملیات", className: "admin-session-actions-column", render: (row: (typeof data.rows)[number]) => <AdminAppointmentStatusMenu appointmentId={row.id} currentStatus={row.status} options={statusOptions} /> }] : []),
      ]} /> : <AdminEmptyState title="جلسه‌ای پیدا نشد" description="فیلتر یا عبارت جست‌وجو را تغییر دهید." />}
