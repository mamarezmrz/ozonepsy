...
export default async function GroupTherapyAdminPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  return <div className="admin-page-stack">
    <AdminPageHeader eyebrow="Product / GroupTherapyProduct" title="گروه‌درمانی" description="ساخت و مدیریت گروه‌های درمانی قابل انتشار در سایت." action={canWrite ? <AdminButton href="/admin/group-therapy/new">گروه‌درمانی جدید</AdminButton> : undefined} />
    <section className="admin-panel-card">
      ...
      {data.rows.length ? <AdminDataTable rows={data.rows} getRowKey={(row) => row.id} columns={[
        { key: "title", label: "عنوان", render: (row) => <Link className="admin-table-link" href={`/admin/group-therapy/${row.id}`}>{row.title}</Link> },
        ...
        { key: "actions", label: "عملیات", render: (row) => <div className="admin-table-actions">
          {canWrite ? <AdminButton href={`/admin/group-therapy/${row.id}`} variant="secondary">ویرایش</AdminButton> : null}
      ...
      ]} /> : <AdminEmptyState title="گروه‌درمانی‌ای پیدا نشد" description="برای ساخت اولین گروه‌درمانی از دکمهٔ بالای صفحه استفاده کنید." action={canWrite ? <AdminButton href="/admin/group-therapy/new">گروه‌درمانی جدید</AdminButton> : undefined} />}
