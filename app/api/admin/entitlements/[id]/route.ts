...
import { revokeAdminEntitlement, updateAdminEntitlementSessions } from "@/lib/admin/entitlements";
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const session = await requireAdminSession();
    requireAdminPermissionFromSession(session, "sessions.manage");
    const result = await revokeAdminEntitlement(session.userId, (await params).id);
    return NextResponse.json({ ok: true, data: result, message: "دسترسی بسته از حساب کاربر برداشته شد." });
  } catch (error) {
    return adminErrorResponse(error);
  }
}
