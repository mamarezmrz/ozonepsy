...
export async function POST(request: Request) {
  ...
  try { const session = await requireAdminSession(); requireAdminPermissionFromSession(session, "courses.write"); const input = adminCourseCreateSchema.parse(await request.json()); return NextResponse.json({ ok: true, message: "دوره ایجاد شد.", data: await createAdminCourse(session.userId, input) }, { status: 201 }); } catch (error) { return adminErrorResponse(error); }
}
