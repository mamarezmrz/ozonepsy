...
export async function listAdminMedia(query: AdminListQuery, visibility?: MediaVisibility, status?: MediaStatus) {
  ...
  const [total, rows] = await Promise.all([
    ...
    prisma.mediaAsset.findMany({
      ...
      take: query.pageSize,
      select: { id: true, originalName: true, mimeType: true, extension: true, size: true, width: true, height: true, visibility: true, status: true, createdAt: true, _count: { select: { products: true, specialists: true, lessons: true, testimonials: true } } },
    }),
  ]);
...
export async function archiveAdminMedia(session: AdminSessionView, id: string, reason: string) {
  ...
  return prisma.$transaction(async (tx) => {
    const before = await tx.mediaAsset.findUnique({ where: { id }, select: { id: true, status: true, visibility: true, originalName: true, _count: { select: { products: true, specialists: true, lessons: true, testimonials: true } } } });
    ...
    const references = before._count.products + before._count.specialists + before._count.lessons + before._count.testimonials;
...
export async function getAdminMedia(id: string, session: AdminSessionView) {
  ...
  const media = await prisma.mediaAsset.findUnique({ where: { id }, select: { id: true, storageKey: true, originalName: true, mimeType: true, extension: true, size: true, width: true, height: true, altText: true, visibility: true, status: true, createdAt: true, _count: { select: { products: true, specialists: true, lessons: true, testimonials: true } } } });
