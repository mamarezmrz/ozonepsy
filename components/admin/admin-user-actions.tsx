...
export function AdminUserActions({ userId, status }: { userId: string; status: UserStatus }) {
  ...
  return <div className="admin-action-stack">
    <div className="admin-table-actions"><button type="button" className={`admin-inline-action${status === "ACTIVE" ? " admin-inline-action-danger" : ""}`} onClick={() => { setConfirming(nextStatus); setReason(""); }} disabled={pending}>{status === "ACTIVE" ? "تعلیق کاربر" : "رفع تعلیق"}</button></div>
    {confirming ? <div className="admin-confirm-panel" role="dialog" aria-label="تأیید تغییر وضعیت"><strong>{confirming === "SUSPENDED" ? "تعلیق کاربر" : "رفع تعلیق کاربر"}</strong><label><span>دلیل</span><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="دلیل این عملیات را وارد کنید…" /></label><div className="admin-table-actions"><button type="button" className="admin-button admin-button-secondary" onClick={() => setConfirming(null)} disabled={pending}>انصراف</button><button type="button" className="admin-button admin-button-danger" onClick={submit} disabled={pending || !reason.trim()}>{pending ? "در حال ذخیره…" : "تأیید"}</button></div>{error ? <p className="admin-inline-error" role="alert">{error}</p> : null}</div> : null}
  </div>;
}
