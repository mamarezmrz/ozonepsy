...
export function AuthForm({mode="login"}:{mode?:"login"|"signup"}){
  ...
  return <form noValidate onSubmit={handleSubmit} className="grid gap-4">{mode==="signup"&&<input required name="displayName" aria-label="نام" placeholder="نام و نام خانوادگی" className="focus-ring rounded-xl border border-[#d8e5e5] px-4 py-3"/>}<input required name="email" type="email" aria-label="ایمیل" placeholder="آدرس ایمیل" className="focus-ring rounded-xl border border-[#d8e5e5] px-4 py-3"/><input required name="password" type="password" aria-label="رمز عبور" placeholder="رمز عبور" className="focus-ring rounded-xl border border-[#d8e5e5] px-4 py-3"/>{error&&<div role="alert" className="rounded-xl bg-[#fff5f5] p-4 text-sm text-[#db4244]">{error}</div>}{message?<div role="status" className="rounded-xl bg-[#ebf7f7] p-4 text-sm font-bold text-[#355859]">{message}</div>:<Button type="submit" disabled={isSubmitting}>{isSubmitting?"لطفاً صبر کنید":mode==="login"?"ورود به حساب":"ساخت حساب"}</Button>}</form>}
